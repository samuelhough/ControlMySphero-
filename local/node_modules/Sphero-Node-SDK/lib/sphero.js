...
Sphero.prototype.getVersioning = function(cb, errCb) {
  var options = {
    device: this.devices.core,
    command: 0x02
  };
  ...
  if(errCb){
    options.error = function(packet){
      errCb(self.parseData(packet))
    }
  }
  this.send(options);
  return this;
...
Sphero.prototype.changeLedColor = function(color, cb){
  cb = cb || noop;
  console.log('Setting color called')
  var data = new Buffer(4);
  data.writeInt16BE(120,1);
  data.writeInt16BE(0,1);
  data.writeInt16BE(55,1);
  data[3] = 0;
